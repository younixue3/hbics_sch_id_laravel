<template>
    <div @mouseover="onHover()" @mouseleave="onLeave()" :class="[column ? 'md:col-span-2' : 'md:col-span-1', rows ? 'md:row-span-2' : 'md:row-span-1']" class="relative bg-cover h-52 md:h-full bg-center col-span-1 transition-all ease-in-out duration-200 rounded-2xl shadow-lg" :style="{ backgroundImage: `url('${imagecontent}')` }">
        <div class="absolute bg-white w-full bottom-0 transition-all ease-in-out duration-200 rounded-b-2xl overflow-hidden px-7 py-5" :class="showup ? 'h-52' : 'md:h-32'">
