<template>
    <Transition name="fade-rdev">
        <div v-if="$store.state.modal.modaladd.status" class="fixed w-screen h-screen bg-black bg-opacity-25 backdrop-blur-sm flex z-50 transition-all ease-in-out duration-300 text-center" :class="$store.state.modal.modaladd.status ? 'opacity-100' : 'opacity-0'">
            <div class="w-[35rem] bg-white m-auto rounded-3xl relative">
                ...
                <div class="p-5 w-full">
                    <h1>Add Data</h1>
                    <form id="formAdd" :action="$store.state.modalvalidation.url.url_req" enctype="multipart/form-data" name="formAdd" method="POST">
